...
// Use env vars only (no hardcoded URL to avoid Netlify secret scans).
const GAS_URL =
...
exports.handler = async (event) => {
  ...
  const limit = Math.max(
    1,
    Math.min(500, parseInt(event.queryStringParameters?.limit || "50", 10) || 50)
  );

  if (!GAS_URL) {
    return respond(500, {
      ok: false,
      error: "Missing GAS_BRIDGE_URL (or GAS_WEB_APP_URL/SHEETS_BRIDGE_URL) env var",
    });
  }
