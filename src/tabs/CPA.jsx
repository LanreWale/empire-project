...
async function listCPAAccounts(){const u=new URL(GAS);u.searchParams.set("action","listCPAAccounts");u.searchParams.set("pass",PASS);
const r=await fetch(u);const j=await r.json();if(!r.ok||j?.ok===false)throw new Error(j?.error||`HTTP ${r.status}`);return j.data||j.items||[];}
export default function CPA(){const [rows,setRows]=React.useState(null),[err,setErr]=React.useState("");
...
return (<div style={{padding:16}}>
  ...
  {rows&&rows.length>0&&(<table style={{width:"100%",borderCollapse:"collapse"}}>
    <thead><tr><th style={{textAlign:"left",borderBottom:"1px solid #213"}}>Network</th>
    <th style={{textAlign:"left",borderBottom:"1px solid #213"}}>Account</th>
    <th style={{textAlign:"left",borderBottom:"1px solid #213"}}>Status</th></tr></thead>
