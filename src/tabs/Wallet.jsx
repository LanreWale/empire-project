...
async function walletOverview(){const u=new URL(GAS);u.searchParams.set("action","walletOverview");u.searchParams.set("pass",PASS);
const r=await fetch(u);const j=await r.json();if(!r.ok||j?.ok===false)throw new Error(j?.error||`HTTP ${r.status}`);return j.data||j;}
export default function Wallet(){const [data,setData]=React.useState(null),[err,setErr]=React.useState("");
...
function Stat({label,value}){return(<div style={{background:"#0b1320",border:"1px solid #15243d",borderRadius:12,padding:14}}>
