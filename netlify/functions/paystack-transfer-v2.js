 ...
 * - USD->NGN via USD_NGN_RATE (default 1600). Sent to Paystack in kobo.
 * - Add ?debug=1 to echo what the function actually received.
 */
...
const USD_NGN_RATE        = Number(process.env.USD_NGN_RATE || 1600);
...
exports.handler = async (event) => {
  try {
    ...
    if (event.queryStringParameters?.debug === "1") {
      return RESP(200, {
        ok: true, debug: true,
        headers: event.headers, base64: !!event.isBase64Encoded,
        raw: parsed.raw, parsed: b,
