...
async function usersOverview(){const u=new URL(GAS);u.searchParams.set("action","usersOverview");u.searchParams.set("pass",PASS);
const r=await fetch(u);const j=await r.json();if(!r.ok||j?.ok===false)throw new Error(j?.error||`HTTP ${r.status}`);return j.data||j;}
export default function Users(){const [data,setData]=React.useState(null),[err,setErr]=React.useState("");
